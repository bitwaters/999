...
| provider-data-ingestion | 类型漂移、压缩 raw、超大响应、429、G2/OHLCV schema、无 Key 公共端点 | `src/providers/*.test.ts`；`src/persistence/db.test.ts`；`scripts/coingecko-real-contract.mjs` | 部分通过（CoinGecko 真实合同验收 32/34；SOL G2/G3 WebSocket 两项未在 20 秒窗口收到数据） |
...
| runtime-operations | 10 表边界、health/degrade、backup restore、dirty deploy guard、CI/Docker smoke、长期运行入口 | `src/persistence/db.test.ts`、`src/runtime/*.test.ts`、`src/app/main.ts`、`src/app/provider-probe.ts`、`src/domain/safety.test.ts`、`scripts/deploy.sh`、Docker build/run | 部分通过：`deploy.sh` 已通过 shell 语法检查并在无 `origin` 时安全停止；应用容器已具备长期 runtime foundation，并可由 Compose 等待；真实 provider connectivity/discovery probe 可写入 GMGN 原始事件、Candidate Cycle、初始 SOL/BSC safety classification、CoinGecko 主池/Level1 事件，Armed→G2→规范化 trades、Signal hard-gate 和 Telegram outbox worker 已接入，磁盘高水位会阻止新 discovery，独立原始 Shadow sampler 也可运行。但实时 G2 窗口驱动 Signal/Outcome 的完整长期 Bot 仍未完成 |
...
- CoinGecko 深测：32/34 通过，覆盖 50 池批量、50 token 批量、REST trades、30 秒 base/quote OHLCV、G2/G3 WebSocket、SOL/BSC 和 credits 计数；失败为 SOL G2/G3 WebSocket 在 20 秒窗口内未收到数据，不能据此宣称 SOL live WS 完整。
- Telegram 只读预检：`getMe`、admin private、group 通过；配置的 channel chat 返回 `chat not found`，未发送消息。
- Docker smoke：app/sampler 镜像构建成功；容器内 healthcheck 输出 healthy，SQLite schemaVersion=1；宿主机 healthcheck 因 96.8% 磁盘使用率超过 85% 高水位而诚实失败；dirty deploy guard 在未提交工作树下于 `git pull` 前以 exit=1 拒绝。Compose 现在包含长期 app、Telegram outbox worker 与带健康检查的原始 Shadow sampler，并由 `deploy.sh` 使用 `up -d --wait` 启动；该结果仍不证明实时 G2 窗口驱动 Signal/Outcome 或服务器部署已完成。
