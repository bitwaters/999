...
- 服务器检查：SSH `lumi-server` 可达，主机为 `dwhkmZxyd8sskPaG`；服务器 `/www/wwwroot/999` 已按 Git 快进到 `a247168`，Compose 依据仓库脚本重建，真实 `.env` 配置解析通过，当前 config version 与构建 provenance 均记录 `a247168`，schemaVersion=3，配置 hash 与本地按同一 ID 解析结果一致；Shadow sampler 状态为 `running/healthy`，采样卷持久化正常。迁移后的 BSC 未解析候选已持久化 `pool_retry_attempt/pool_retry_at`；最新 runtime health 为 `failed`（provider/safety/telegram/sqlite/event_loop 正常、Level 1 failed、G2 unknown、磁盘高水位为 false），deploy.sh 因健康门禁正确返回失败；这不等同于 production 放行。
- 服务器最新采样只读审计：665 provider calls（GMGN 431/431、CoinGecko REST 218/218、WebSocket 16/16 均无失败），累计 BSC indexing 129/3576=3.61%、唯一 token 129/1989=6.49%，SOL 2227/2323=95.87%、唯一 token 2227/2233=99.73%；退避后最近 5 分钟 BSC 10/250=4.00%、唯一 token 10/212=4.72%，SOL 172/172=100%、唯一 token 172/172=100%。BSC 辅助采样来源 `trenches/new_creation` 25/2086、辅助采样 `trending` 102/166；credits 4 个采样点且剩余 391451。Outcome 表当前为 0 行、参数敏感性不可估计，预算需人工复核，S1 全部保持关闭；replay/report guard 输出 `writes=[]`；审计结论为 `hold_shadow`，不支持 production。
