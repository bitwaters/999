...
- 服务器检查：SSH `lumi-server` 可达，主机为 `dwhkmZxyd8sskPaG`；服务器 `/www/wwwroot/999` 已按 Git 快进到 `3f48de7`，Compose 依据仓库脚本重建，真实 `.env` 配置解析通过，当前 config version 与构建 provenance 均记录 `3f48de7`，schemaVersion=3，配置 hash 与本地按同一 ID 解析结果一致；GMGN ban 后 sampler 曾因缺少请求超时停滞，部署 timeout 修复后恢复 `running/healthy`，采样卷持久化正常。迁移后的 BSC 未解析候选已持久化 `pool_retry_attempt/pool_retry_at`；最新 runtime health 为 `failed`（provider/safety/telegram/sqlite/event_loop 正常、Level 1 failed、G2 unknown、磁盘高水位为 false），deploy.sh 因健康门禁正确返回失败；这不等同于 production 放行。
- 服务器最新采样只读审计：764 provider calls（GMGN 492/488、CoinGecko REST 252/252、WebSocket 20/20），累计 BSC indexing 146/4126=3.54%、唯一 token 146/2309=6.32%，SOL 2472/2570=96.19%、唯一 token 2472/2478=99.76%；退避后最近 5 分钟 BSC 3/200=1.50%、唯一 token 3/151=1.99%，SOL 62/62=100%。BSC 辅助采样来源 `trenches/new_creation` 27/2386、辅助采样 `trending` 117/178；credits 6 个采样点且剩余 389971。Outcome 表当前为 0 行、参数敏感性不可估计，预算需人工复核，S1 全部保持关闭；replay/report guard 输出 `writes=[]`；审计结论为 `hold_shadow`，不支持 production。
