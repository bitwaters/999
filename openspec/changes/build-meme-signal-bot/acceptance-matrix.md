...
| runtime-operations | 10 表边界、health/degrade、backup restore、dirty deploy guard、CI/Docker smoke、长期运行入口 | `src/persistence/db.test.ts`、`src/runtime/*.test.ts`、`src/app/main.ts`、`scripts/deploy.sh`、Docker build/run | 部分通过：应用容器已具备长期 runtime foundation，并可由 Compose 等待；独立原始 Shadow sampler 也可运行。但 provider/pipeline/delivery/Outcome 尚未接入，仍不是完整长期 Signal/Outcome Bot |
