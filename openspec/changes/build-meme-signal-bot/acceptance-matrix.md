...
| runtime-operations | 10 表边界、health/degrade、backup restore、dirty deploy guard、CI/Docker smoke、长期运行入口 | `src/persistence/db.test.ts`、`src/runtime/*.test.ts`、`src/app/main.ts`、`src/app/provider-probe.ts`、`src/domain/safety.test.ts`、`scripts/deploy.sh`、Docker build/run | 部分通过：应用容器已具备长期 runtime foundation，并可由 Compose 等待；真实 provider connectivity/discovery probe 可写入 GMGN 原始事件、Candidate Cycle、初始 SOL/BSC safety classification、CoinGecko 主池/Level1 事件，G2 transport、Signal hard-gate 和 Telegram outbox worker 已接入，磁盘高水位会阻止新 discovery，独立原始 Shadow sampler 也可运行。但 G2 client 尚未接入 Armed 生命周期，仍不是完整长期 Signal/Outcome Bot |
...
- Docker smoke：app/sampler 镜像构建成功；容器内 healthcheck 输出 healthy，SQLite schemaVersion=1；宿主机 healthcheck 因 96.8% 磁盘使用率超过 85% 高水位而诚实失败；dirty deploy guard 在未提交工作树下于 `git pull` 前以 exit=1 拒绝。Compose 现在包含长期 app、Telegram outbox worker 与带健康检查的原始 Shadow sampler，并由 `deploy.sh` 使用 `up -d --wait` 启动；该结果仍不证明 G2 Armed 生命周期、完整 Signal/Outcome Bot 或服务器部署已完成。
