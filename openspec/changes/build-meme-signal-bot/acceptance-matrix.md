...
- 服务器检查：SSH `lumi-server` 可达，主机为 `dwhkmZxyd8sskPaG`；服务器 `/www/wwwroot/999` 已按 Git 快进到 `c3262bc`，Compose 依据仓库脚本重建，真实 `.env` 配置解析通过，当前 config version 记录 `c3262bc`、schemaVersion=2，配置 hash 与本地按同一 ID 解析结果一致；重启后 Shadow sampler 状态为 `running`，状态文件记录 5040 candidate observations、2663 indexing attempts、1045 token pools、1379 pool snapshots、206 WebSocket events、303 provider calls、无 sampler error，采样卷持久化正常。app 已启动但 runtime health 因 Level 1 不完整而保持 `failed`（provider/safety/telegram 已 ok，level1 failed、g2 unknown，磁盘高水位为 false），deploy.sh 因健康门禁正确返回失败；replay/report clean-main guard 通过；这不等同于 production 放行。
- 服务器最新采样只读审计：303 provider calls（GMGN 192/192、CoinGecko REST 99/99、WebSocket 12/12 均无失败），累计 BSC indexing 60/1626=3.69%、SOL 985/1037=94.99%；最近 5 分钟切片 BSC 4/350=1.14%、SOL 194/216=89.81%，WebSocket events 206，credits 3 个采样点且剩余 392031。Outcome 表当前为 0 行、参数敏感性不可估计，预算需人工复核，S1 全部保持关闭；审计结论为 `hold_shadow`，不支持 production。
