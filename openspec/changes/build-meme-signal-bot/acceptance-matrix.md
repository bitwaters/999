...
- GMGN 预检：SOL/BSC trending 1m/5m、hot-searches 通过；深测大部分接口通过，但脚本请求了供应商不支持的 30m/3h interval（400），随后 BSC `token.traders` 触发真实 `RATE_LIMIT_BANNED`，按供应商提示停止请求。未继续重试。
