...
| runtime-operations | 10 表边界、health/degrade、backup restore、dirty deploy guard、CI/Docker smoke、长期运行入口 | `src/persistence/db.test.ts`、`src/runtime/*.test.ts`、`scripts/deploy.sh` dirty guard、Docker build/run | 部分通过：Docker image build 与一次性 container healthcheck 通过，但没有正式长期 Bot runtime，`deploy.sh` 未执行 Compose start |
...
- Docker smoke：`meme-signal-bot:local` 构建成功；一次性容器 healthcheck 通过，SQLite schemaVersion=1、status=healthy；dirty deploy guard 在未提交工作树下于 `git pull` 前以 exit=1 拒绝。该结果不证明长期 Bot 已启动，当前 `deploy.sh` 未执行 Compose start，需正式 runtime 后重验。
