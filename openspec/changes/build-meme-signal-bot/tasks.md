...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（Shadow 已持续产生真实 Signal/Outcome，工程漏斗、磁盘、credits、replay 让步和部署纪律均有证据；但真实 Outcome review 发现旧 G2 parser 把 `toq/to` 误作 USD 价格并把 `toq` 误作 USD 成交额，且 60m candle 可能错过 cutoff。历史样本保留但不得用于产品验收，本地修复需部署为新 cohort 后重新验证，因此本项仍不能勾选）
