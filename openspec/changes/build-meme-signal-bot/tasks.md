...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（`5b520c8` 已按 Git 流程拉取并由 deploy.sh 冷启动部署；app 与 sampler 均 `running/healthy`，runtime 最终 `healthy`，组件全部 `ok`。实测发现并修复 live G2 的 `ty=b/s`、numeric amount 编码，以及重启后 Armed 订阅恢复；修复后重启窗口内 G2 raw 从 1,586 增至 1,587，normalized `trades` 从 0 增至 1。signals/outcomes 仍为 0，Outcome、replay 让步和更长周期重启恢复仍未完成；用户要求暂跳过 sampler 卡顿原因排查）
...
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（服务器仓库与运行镜像 provenance 均为 `5b520c8`，config hash=`30b7b273…`、schemaVersion=3、Shadow 锚点核对一致；app 与 sampler 均 healthy，冷启动 deploy.sh 健康门禁通过，runtime health 通过，但 signals/outcomes 尚无样本，累计 Outcome、replay 让步和 production 前置评审尚未完成，继续保持 Shadow，不满足 production 放行）
