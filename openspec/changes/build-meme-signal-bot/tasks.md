...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（运行中的 app 镜像由代码提交 `3f48de7` 构建，schemaVersion=3；GMGN ban 后 sampler 曾因无请求超时停滞，已修复并恢复 `healthy`，当前 Outcome=0 行，replay/report guard 无写入；最新 app health 为 `failed`，Level 1 failed、G2 failed；持续完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 492 calls/4 failures、CoinGecko REST 252/0、WebSocket 20/0；累计 BSC indexing 146/4126=3.54%、唯一 token 146/2309=6.32%，SOL 2472/2570=96.19%、唯一 token 2472/2478=99.76%；退避后最近 5 分钟 BSC 3/200=1.50%、唯一 token 3/151=1.99%，SOL 62/62=100%；BSC 辅助采样来源 `trenches/new_creation` 为 27/2386，辅助采样 `trending` 为 117/178；credits 6 个采样点且剩余 389971；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。已修复未解析池的持久化指数退避和 GMGN 请求永久停滞，避免将 CoinGecko 覆盖延迟重复计入尝试率）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（服务器仓库已按 Git 快进到文档提交 `1bbcf4d`；运行中的 app 构建 provenance 为代码提交 `3f48de7`，config hash/schemaVersion=3/Shadow 锚点均核对通过；迁移 003 的 BSC 重试字段已生效，sampler 已恢复 healthy，runtime health 为 `failed`（Level 1 failed、G2 failed），保持 Shadow，不满足 production 放行）
