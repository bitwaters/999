...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（提交 `3aac3e3` 已按 Git 流程拉取并由 deploy.sh 重建；app 与 sampler 均 `running/healthy`，runtime 最终 `healthy`，组件全部 `ok`，G2 未启用时不再误报 `unknown`。Outcome、replay 让步和更长周期重启恢复仍未完成；用户要求暂跳过 sampler 卡顿原因排查）
...
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（服务器仓库与运行镜像 provenance 均为 `3aac3e3`，config hash=`30b7b273…`、schemaVersion=3、Shadow 锚点核对一致；app 与 sampler 均 healthy，runtime health 通过，但累计 Outcome、replay 让步和 production 前置评审尚未完成，继续保持 Shadow，不满足 production 放行）
