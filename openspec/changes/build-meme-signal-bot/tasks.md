...
- [ ] 10.6 创建简短 deploy.sh：dirty 检查、ff-only pull、Compose rebuild/start、健康检查；不得包含服务器代码/配置修改逻辑。（应用容器现已具备长期 runtime foundation、真实 provider connectivity/discovery probe 和初始 SOL/BSC safety classification；配置版本、SQLite、GMGN Trending/Hot Searches 原始事件、Candidate Cycle、备份、事件循环监测、心跳和 SIGTERM 停机均已接入，独立原始 Shadow sampler 也可等待启动。但完整 safety freshness/Level1/G2/pipeline/delivery/Outcome 尚未接入，Signal Bot start 语义仍未满足）
