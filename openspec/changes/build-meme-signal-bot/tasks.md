...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器已按 `9441285` 通过仓库脚本 Compose 重建，schemaVersion=3，sampler healthy 且继续运行；当前采样审查累计 665 provider calls、218 WebSocket events，Outcome=0 行，replay/report guard 无写入；app health 为 `failed`，Level 1 failed、G2 unknown；持续完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 431/431、CoinGecko REST 218/218、WebSocket 16/16 无失败；累计 BSC indexing 129/3576=3.61%、唯一 token 129/1989=6.49%，SOL 2227/2323=95.87%、唯一 token 2227/2233=99.73%；退避后最近 5 分钟 BSC 10/250=4.00%、唯一 token 10/212=4.72%，SOL 172/172=100%、唯一 token 172/172=100%；BSC 辅助采样来源 `trenches/new_creation` 为 25/2086，辅助采样 `trending` 为 102/166；credits 4 个采样点且剩余 391451；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。已修复未解析池的持久化指数退避，避免将 CoinGecko 覆盖延迟重复计入尝试率）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（main 已推送且服务器已按 Git 拉取到 `a247168`，构建 provenance/config hash/schemaVersion=3/Shadow 锚点均核对通过；迁移 003 的 BSC 重试字段已生效，runtime health 为 `failed`（Level 1 failed、G2 unknown），保持 Shadow，不满足 production 放行）
