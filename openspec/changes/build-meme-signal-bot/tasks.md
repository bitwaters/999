...
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 532 calls/4 failures、CoinGecko REST 272/0、WebSocket 20/0；累计 BSC indexing 163/4476=3.64%、唯一 token 163/2489=6.55%，SOL 2670/2768=96.46%、唯一 token 2670/2676=99.78%；退避后最近 5 分钟 BSC 14/300=4.67%、唯一 token 14/218=6.42%，SOL 186/186=100%；BSC 辅助采样来源 `trenches/new_creation` 为 30/2542，辅助采样 `trending` 为 131/187；credits 6 个采样点且剩余 389971；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。已修复未解析池的持久化指数退避和 GMGN 请求永久停滞，避免将 CoinGecko 覆盖延迟重复计入尝试率）
