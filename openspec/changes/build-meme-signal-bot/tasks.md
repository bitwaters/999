...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器已按 `3f48de7` 通过仓库脚本 Compose 重建，schemaVersion=3；GMGN ban 后 sampler 曾因无请求超时停滞，已修复并恢复 `healthy`，当前 Outcome=0 行，replay/report guard 无写入；最新 app health 为 `failed`，Level 1 failed、G2 unknown；持续完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成）
...
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（main 已推送且服务器已按 Git 拉取到 `3f48de7`，构建 provenance/config hash/schemaVersion=3/Shadow 锚点均核对通过；迁移 003 的 BSC 重试字段已生效，sampler 已恢复 healthy，runtime health 为 `failed`（Level 1 failed、G2 unknown），保持 Shadow，不满足 production 放行）
