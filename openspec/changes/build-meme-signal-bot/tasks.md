...
- [ ] 11.3 使用私人 GMGN Key 固化 SOL/BSC 脱敏 fixture，并验证 RPM、并发、reset、封禁、BSC ownership/open-source/tax 与比例量纲。（服务器曾完成 `.env` GMGN Key 合同测试 52/52；通过 `GMGN_FIXTURE_DIR` 生成 6 个脱敏 fixture，鉴权字段审查未发现；有界负载并发 2/2、5.2 秒 pacing 4/4 通过。最终合同重跑在 `token.holders.sol` 收到真实 `HTTP 429 RATE_LIMIT_BANNED` 并返回 reset 时间，暴露合同脚本对 ban 仍重试的问题；已在 `a247168` 修复为 429/ban 立即停止并增加自动测试。真实 ban 已证实，但 reset 边界未主动触发验证，故本项仍不勾选）
...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（运行中的 app 镜像由提交 `909642b` 构建，其代码相对 `3f48de7` 未变化，仅包含验收文档更新，schemaVersion=3；GMGN ban 后 sampler 曾因无请求超时停滞，已修复并恢复 `healthy`，当前 Outcome=0 行，replay/report guard 无写入；最新 sampler 为 `running/healthy`，app runtime 只读快照为 provider/safety/Telegram/SQLite/event-loop 正常、Level 1 failed、G2 unknown，BSC 候选 1,866/已解析池 1,476/安全通过 1,670，说明当前是候选级 Level 1 部分覆盖而非 API 故障；app health 为 `failed`，Level 1/G2 runtime gate 未通过；持续完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 640 calls/4 failures、CoinGecko REST 326/0、WebSocket 20/0；累计窗口约 111.7 分钟，全量 BSC indexing 192/5376=3.57%、唯一 token 192/3018=6.36%，SOL 3203/3301=97.03%、唯一 token 3203/3209=99.81%；线上主发现源 `trending + hot-searches` 合并覆盖 BSC 155/217=71.43%、SOL 414/414=100%；退避后最近 5 分钟 BSC 7/300=2.33%、唯一 token 7/224=3.13%，SOL 172/172=100%；BSC 辅助采样来源 `trenches/new_creation` 为 37/3046，主源 `trending` 为 153/209、`hot-searches` 为 94/145；credits 6 个采样点且剩余 389971；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。已修复未解析池的持久化指数退避、GMGN 请求永久停滞，并将审计生产覆盖口径与辅助 trenches 分离，避免把诊断源误当线上主源）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（本次服务器部署使用提交 `909642b`；运行中的 app 构建 provenance 为 `909642b`，其代码相对 `3f48de7` 未变化，config hash/schemaVersion=3/Shadow 锚点均核对通过；迁移 003 的 BSC 重试字段已生效，sampler 已恢复 healthy，runtime health 为 `failed`（Level 1/G2 runtime gate 未通过），deploy.sh 因健康门禁退出，保持 Shadow，不满足 production 放行）
