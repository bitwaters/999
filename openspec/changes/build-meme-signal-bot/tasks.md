...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（Shadow 已持续产生真实 Signal/Outcome，工程漏斗、磁盘、credits、replay 让步和部署纪律均有证据；旧 G2 USD 量纲、horizon 补采及 cohort 62 的 Armed 工作时钟缺陷均已修复并保留审计。cohort 64 的 Armed/G2/批量时序正常，但对已固化 v2 Outcome 的复审发现非 30 秒对齐 horizon 会被互相矛盾的路径过滤与完整性循环必然标为 `path:missing_complete_coverage`。本地最小修复统一 forward return 与 MFE/MAE 的 eligible evaluation close 终点，不增加 G2 保留时间、表或采集路由；旧 Outcome 保持审计，必须由新 cohort 复验，因此本项仍不能勾选）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；首轮产品评审要求同一 Shadow config version 下每链至少 100 个首次合格锚点已送达且 60m 状态已固化，其中至少 60 个 executable+complete，并按时间前 70%/后 30% 划分且验证段至少 30/18 个对应样本；未通过前保持 Shadow。（旧产品标签受 G2 USD 量纲缺陷影响，不能用于参数敏感性或收益评审；修复后的新 cohort 尚未达到分链 100/60、验证段 30/18 门槛，S1 继续关闭，结论保持 `hold_shadow`）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（当前仍为 Shadow/admin anchor，S0 production verified 标志仍为 false，修复后的产品样本门槛未满足；继续保持 Shadow，明确不满足 production 放行）
