...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（运行中的 app 镜像由提交 `8bcb81c` 构建，config hash/schemaVersion=3/Shadow 锚点核对一致；sampler 配置已采用 120 分钟 pending TTL，当前 app runtime 只读快照为 provider/safety/Telegram/SQLite/event-loop/Level 1 正常、G2 unknown、runtime degraded，Outcome=0 行，replay/report guard 无写入。长期完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成；用户要求暂跳过 sampler 卡顿原因排查）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 728 calls/5 failures、CoinGecko REST 372/0、WebSocket 24/0；累计窗口约 126.5 分钟，全量 BSC indexing 218/6126=3.56%、唯一 token 218/3444=6.33%，SOL 3586/3690=97.18%、唯一 token 3586/3592=99.83%；线上主发现源 `trending + hot-searches` 合并覆盖 BSC 175/237=73.84%、SOL 462/462=100%；最近验证切片 BSC 8/300=2.67%、唯一 token 8/226=3.54%，SOL 134/134=100%；BSC 辅助采样来源 `trenches/new_creation` 为 44/3452，主源 `trending` 为 170/225、`hot-searches` 为 107/158；credits 8 个采样点且剩余 388385；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。sampler 卡顿原因按用户要求暂不展开）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（本次服务器部署使用提交 `8bcb81c`；运行中的 app 构建 provenance 为 `8bcb81c`，config hash/schemaVersion=3/Shadow 锚点均核对通过；迁移 003 的 BSC 重试字段与 pending TTL=120 分钟配置已生效，runtime health 为 `degraded`（G2 unknown），deploy.sh 因健康门禁退出，保持 Shadow，不满足 production 放行）
