...
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 300/300、CoinGecko REST 154/154、WebSocket 16/16 无失败；累计 BSC indexing 83/2526=3.29%、唯一 token 83/1301=6.38%，SOL 1537/1629=94.35%、唯一 token 1537/1542=99.68%；最近 5 分钟 BSC 9/300=3.00%、唯一 token 9/180=5.00%，SOL 176/183=96.17%、唯一 token 176/180=97.78%；BSC `trenches` 14/1426、`trending` 69/131；credits 4 个采样点且剩余 391451；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。已修复未解析池的持久化指数退避，避免将 CoinGecko 覆盖延迟重复计入尝试率）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（main 已推送且服务器已按 Git 拉取到 `ee42f27`，构建 provenance/config hash/schemaVersion=3/Shadow 锚点均核对通过；迁移 003 的 BSC 重试字段已生效，runtime health 仍为 `failed`，保持 Shadow，不满足 production 放行）
