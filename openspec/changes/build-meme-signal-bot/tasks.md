...
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 614 calls/4 failures、CoinGecko REST 312/0、WebSocket 20/0；累计窗口约 107.5 分钟，BSC indexing 186/5126=3.63%、唯一 token 186/2885=6.45%，SOL 3081/3179=96.92%、唯一 token 3081/3087=99.81%；退避后最近 5 分钟 BSC 8/250=3.20%、唯一 token 8/211=3.79%，SOL 167/167=100%；BSC 辅助采样来源 `trenches/new_creation` 为 36/2926，辅助采样 `trending` 为 148/203；credits 6 个采样点且剩余 389971；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。已修复未解析池的持久化指数退避和 GMGN 请求永久停滞，避免将 CoinGecko 覆盖延迟重复计入尝试率）
