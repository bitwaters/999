...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器已按 `ee42f27` 通过仓库脚本 Compose 重建，schemaVersion=3，sampler healthy 且重启后继续运行；当前采样审查为 470 provider calls、218 WebSocket events，Outcome=0 行；app health 仍为 `failed`，最新快照仅确认 sqlite/event_loop 正常且 Telegram 为 failed，Level 1/G2 尚未形成可放行证据；持续完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：GMGN 320/320、CoinGecko REST 164/164、WebSocket 16/16 无失败；累计 BSC indexing 87/2676=3.25%、唯一 token 87/1412=6.16%，SOL 1647/1740=94.66%、唯一 token 1647/1653=99.64%；退避后最近 5 分钟 BSC 9/250=3.60%、唯一 token 9/211=4.27%，SOL 174/175=99.43%、唯一 token 174/175=99.43%；BSC 辅助采样来源 `trenches/new_creation` 为 17/1546，辅助采样 `trending` 为 70/133；credits 4 个采样点且剩余 391451；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`。已修复未解析池的持久化指数退避，避免将 CoinGecko 覆盖延迟重复计入尝试率）
