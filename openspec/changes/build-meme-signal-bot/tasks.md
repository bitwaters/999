...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（本地已实现并测试 sampler 的主源优先调度、持久化 `first_seen_at/last_seen_at/next_retry_at`、到期后再限额和原因分类；Docker Compose 构建与容器语法 smoke 通过。服务器部署、长期完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成；用户要求暂跳过 sampler 卡顿原因排查）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（已确认旧 sampler 在 `continuous-sampler.mjs:441` 先对共享主源/辅助源队列执行 LIMIT，再判断重试时间，造成 BSC Trenches 挤出主源；本地修复为每链主源优先、辅助源仅填充剩余额度，并新增 `not_attempted/token_absent/token_present_no_top_pool/pool_relationship_missing/included_pool_missing/invalid_pool_identity/resolved/provider_error` 分层与 5/15/30/60/120 分钟成熟窗口审计。修复尚未部署到服务器，新的累计样本、参数敏感性、预算评审仍未完成，结论保持 `hold_shadow`）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（本地调度修复已通过 102 个测试、build、lint、typecheck、OpenSpec 相关脚本检查和 Docker Compose 构建；尚未完成本次提交后的服务器拉取/重建/健康核对，仍保持 Shadow）
