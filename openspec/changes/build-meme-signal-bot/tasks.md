...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（Shadow 已持续产生真实 Signal/Outcome，工程漏斗、磁盘、credits、replay 让步和部署纪律均有证据；旧 G2 USD 量纲、horizon 补采及 cohort 62 的 Armed 工作时钟缺陷均已修复并保留审计。cohort 63 证明 Armed 首次批量刷新已按 45 秒 due 执行，但交叉复审发现批量完成时若尚无成交，首笔 G2 不会立即把更新的筛选快照补成完整 Level 1，仍可能等到下一轮刷新或在租约尾部错失机会。本地已增加只对更新批量快照执行一次的 G2 补全与基线轮转；必须经新 cohort 真实复验，因此本项仍不能勾选）
