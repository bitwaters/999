...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器严格按“本地修改→GitHub main→deploy.sh ff-only pull”部署至 `3ff5b56`；app/sampler healthy、app restart=0，真实 Level 1 probe 每轮 attempted=100、complete=98–100、status=ok。固定 raw replay run 4 扫描 4,673 条 raw，得到 629=0 full/4 partial/625 unavailable，耗时 363.9 秒；replay NI=10、app NI=0，全程 heartbeat healthy/event_loop ok，live 表无污染且临时文件清空。完整性、磁盘与 replay 让步实现已验证，但 signals/outcomes 仍为 0，真实可执行率与 Outcome 样本尚不存在，因此本项整体不能勾选）
...
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（最新部署 commit=`3ff5b56`、config hash=`30b7b273…`、schemaVersion=3，仍为 Shadow/admin anchor；S0 production verified 标志仍为 false，signals/outcomes 为 0。继续保持 Shadow，明确不满足 production 放行）
