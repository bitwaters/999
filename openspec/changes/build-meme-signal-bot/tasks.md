...
- [x] 9.6 实现 SQLite online backup page batch、一致临时副本、磁盘预检、run-id 清理、live backlog 让步和小批结果写回。（正式 wrapper 在 run 创建后执行 online backup，只读 snapshot；按 runtime SQLite/disk/G2 queue 水位让步，不再把 cutoff 后正常 raw 新增误判为 backlog；失败统一写回 run，临时副本 finally 清理）
...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器严格按“本地修改→GitHub main→deploy.sh ff-only pull”部署至 `1ec03bf`；app/sampler healthy、app restart=0，真实 Level 1 probe attempted=100/complete=98/incomplete=2/status=ok，runtime 全组件 ok。Candidate lifecycle、age/window、G2 解析与局部 Level 1 缺口隔离均已生效；signals/outcomes 仍为 0，新 replay 代码尚待部署后用真实 raw 验证，因此可执行率、Outcome 与本项整体仍不能验收）
...
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（最新部署 commit=`1ec03bf`、config hash=`30b7b273…`、schemaVersion=3，仍为 Shadow/admin anchor；S0 production verified 标志仍为 false，signals/outcomes 为 0。继续保持 Shadow，明确不满足 production 放行）
