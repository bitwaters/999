...
- [ ] 10.6 创建简短 deploy.sh：dirty 检查、ff-only pull、Compose rebuild/start、健康检查；不得包含服务器代码/配置修改逻辑。（应用容器现已具备长期 runtime foundation 和真实 provider connectivity probe；配置版本、SQLite、GMGN 原始 discovery probe、备份、事件循环监测、心跳和 SIGTERM 停机均已接入，独立原始 Shadow sampler 也可等待启动。但完整 discovery/safety/Level1/G2/pipeline/delivery/Outcome 尚未接入，Signal Bot start 语义仍未满足）
