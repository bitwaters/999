...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器已完成 Git 克隆、Compose 校验、生产镜像构建、临时凭据 healthcheck 烟测，以及 Docker 内 lint/typecheck/88 项测试；真实 `.env`、长期 Shadow 采样和运行指标仍待配置）
