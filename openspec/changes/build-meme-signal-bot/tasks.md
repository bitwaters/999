...
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器已按 `c3262bc` 通过仓库脚本 Compose 重建，重启后 sampler 继续运行；当前 5040 candidate observations、2663 indexing attempts、1045 token pools、1379 pool snapshots、206 WebSocket events、303 provider calls，Outcome=0 行；app health 因 Level 1 不完整保持失败，持续完整率、可执行率、Outcome、replay 让步和更长周期重启恢复仍未完成）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（服务器最新只读评审：总样本 303 provider calls，GMGN 192/192、CoinGecko REST 99/99、WebSocket 12/12 无失败；累计 BSC indexing 60/1626=3.69%、SOL 985/1037=94.99%，最近 5 分钟 BSC 4/350=1.14%、SOL 194/216=89.81%；WebSocket events 206，credits 3 个采样点且剩余 392031；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论为 `hold_shadow`）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（main 已推送且服务器已按 Git 拉取到 `c3262bc`、构建 provenance/config hash/schemaVersion/Shadow 锚点均核对通过，replay/report clean-main guard 通过；runtime health 仍因 Level 1/G2 不完整失败，保持 Shadow，不满足 production 放行）
