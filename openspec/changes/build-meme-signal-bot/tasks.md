...
- [x] 9.3 从 raw discovery 按模拟 decision_time 重建 Candidate Cycle、年龄、TTL 与冷却，不复用 live candidate_id。（`src/replay/simulator.ts` 按链独立 TTL、`cooldown + max TTL` warmup、simulated time、age/window 与跨 Cycle 冷却重算；warmup 只建立状态，不输出区间外结果）
- [x] 9.4 从 raw safety、pool/Level 1、G2、OHLCV 使用当前 parser/config 直接重建主池和全部 replay 证据，不复用或写入 live 派生表。（`provider-events.ts` 从 raw 分类；simulator 复用正式 safety、pool、Level 1、G2、OHLCV parser，主池在 Cycle 内固定）
- [x] 9.5 实现严格 observed-at 时间线、simulated confirmation/delivery、dispatch guard、entry、Outcome 和 full/partial/unavailable。（simulator 复用 ACE/age/dispatch/entry/Outcome；`data_end` 只限制新确认，`data_cutoff` 继续提供 entry/Outcome 证据；G2 无法证明连续覆盖时保守 partial，不把静默当作无成交）
- [x] 9.6 实现 SQLite online backup page batch、一致临时副本、磁盘预检、run-id 清理、live backlog 让步和小批结果写回。（正式 wrapper 在 run 创建后执行 online backup，只读 snapshot；按 runtime SQLite/event-loop/disk/G2 queue 水位让步，不再把 cutoff 后正常 raw 新增误判为 backlog；replay 进程固定 NI=10 让在线 app 优先，失败统一写回 run，snapshot/WAL/SHM finally 清理）
- [x] 9.7 测试 TTL/参数/parser 改变、无历史 G2、cutoff 未覆盖 horizon、无前视、actual/simulated 隔离和 replay 不生成 Outbox。（新增 raw event 分类、同规则 simulator、参数敏感性、缺 G2、prepared-run failure；版本化容器已验证 current config 与 saved-version + `--set`，只写 replay/config 允许表）
...
- [x] 11.4 实测 CoinGecko G2 quote、多 leg、重复、乱序、重连、每 socket/并发上限、`/key` 总余额差分和 50 池尾延迟；REST/G2 并发且无分项账单，不声称已验证 per-message G2 成本。（按池列表 m5 活跃度选择合同测试池后，真实合同验收 34/34；GMGN 52/52）
- [ ] 11.5 在服务器以 Shadow/admin anchor 部署，持续采集并验证完整率、索引延迟、可执行率、Outcome、credits、磁盘与 replay 让步。（服务器严格按“本地修改→GitHub main→deploy.sh ff-only pull”部署；确认刷新修复已至 `3709d29`，app/sampler healthy、restart=0，首轮 Level 1 attempted=100、complete=100，部署后近 5 分钟供应商/刷新错误为 0。固定 raw replay run 4 扫描 4,673 条 raw，得到 629=0 full/4 partial/625 unavailable，耗时 363.9 秒；replay NI=10、app NI=0，全程 heartbeat healthy/event_loop ok，live 表无污染且临时文件清空。完整性、磁盘与 replay 让步实现已验证，但 signals/outcomes 仍为 0，真实可执行率与 Outcome 样本尚不存在，因此本项整体不能勾选）
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；首轮产品评审要求同一 Shadow config version 下每链至少 100 个首次合格锚点已送达且 60m 状态已固化，其中至少 60 个 executable+complete，并按时间前 70%/后 30% 划分且验证段至少 30/18 个对应样本；未通过前保持 Shadow。（最新只读审计：采样 1,553 calls、累计约 172.3 分钟；BSC 主源 294 个候选、266 个已查询、28 个 `not_attempted`，调度覆盖 90.48%，已查询解析率 88.35%，全体解析率 79.93%；SOL 为 615/615/615，100%/100%。BSC 5/15/30/60/120 分钟成熟解析率为 68.06%/67.64%/65.35%/66.97%/70%，SOL 五档均为 100%；GMGN 1004 calls/5 failures，CoinGecko REST 513/0、WebSocket 36/0；credits 11 个采样点且剩余 386272；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论保持 `hold_shadow`。原因分类和新统计口径已生效，sampler 卡顿原因按用户要求暂不展开）
- [ ] 11.7 production 配置经本地修改、main/CI 和服务器拉取部署后，核对 commit/config hash/schema/锚点/健康并记录放行结果。（最新业务代码部署 commit=`3709d29`、config hash=`30b7b273…`、schemaVersion=3，仍为 Shadow/admin anchor；S0 production verified 标志仍为 false，signals/outcomes 为 0。继续保持 Shadow，明确不满足 production 放行）
