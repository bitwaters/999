...
- [ ] 11.6 基于累计样本完成训练/最近验证切片、参数敏感性、预算模拟和 S1 风险政策人工评审；未通过前保持 Shadow。（最新只读审计：采样 1,553 calls、累计约 172.3 分钟；BSC 主源 294 个候选、266 个已查询、28 个 `not_attempted`，调度覆盖 90.48%，已查询解析率 88.35%，全体解析率 79.93%；SOL 为 615/615/615，100%/100%。BSC 5/15/30/60/120 分钟成熟解析率为 68.06%/67.64%/65.35%/66.97%/70%，SOL 五档均为 100%；GMGN 1004 calls/5 failures，CoinGecko REST 513/0、WebSocket 36/0；credits 11 个采样点且剩余 386272；无 Outcome 标签，参数敏感性不可估计，预算需人工复核，S1 全部关闭，结论保持 `hold_shadow`。原因分类和新统计口径已生效，sampler 卡顿原因按用户要求暂不展开）
