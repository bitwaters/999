...
- [ ] 7.2 按 chain + git commit + config version cohort 累计，每链同一 cohort 至少 500 个有效 batch candidates 与 50 个 finalists 后进行首轮工程评审：无 rate/credit defer 样本中 safety-pass due→batch complete P95≤10 秒、reservation→Armed 后 G2 发起 P95≤10 秒，同语料 REST calls 降幅≥80%、本地并发/限流错误造成的 429=0；修复后可用新 cohort 重评但旧异常保留，defer 单列端到端延迟、credits 投影与 backlog，不按自然日清零或强制结束。（cohort 62 的重复 discovery 覆写 Armed 工作时钟缺陷已修复；cohort 63 的 SOL/BSC 有效 batches 与 finalists 已超过 500/50，P95、REST 降幅和 429 指标名义合格，且首个 Armed 刷新均在 45 秒 due 后约 1.5–6.2 秒完成。正式交叉复审仍发现：批量完成时若还没有 G2 成交，新筛选快照不会在首笔 G2 到来时补全，信号会继续报告缺价格基线直到下一轮批量刷新，租约尾部成交可能直接错失。cohort 63 保留但不得承担最终工程放行；本地最小修复为首笔 G2 立即补全且同批快照只轮转一次，必须由新 cohort 复验）
- [ ] 7.3 在真实 Signal/Outcome 样本不足时仅确认工程指标并继续 Shadow；首轮产品评审要求同一 Shadow config version 下每链至少 100 个首次合格锚点已送达且 60m 状态已固化，其中至少 60 个 executable+complete，并按时间前 70%/后 30% 划分且验证段至少 30/18 个对应样本；未达标冻结收益参数，达标后评审 Wilson 95% 区间、切片稳定性、完整率、尾部亏损和预算，失败则通过 Git main/deploy.sh 回滚。（旧 G2 USD 量纲与 horizon 补采缺陷已由 parser/coverage v2 和强制补采隔离，但真实 v2 Outcome 又证明非 30 秒对齐 horizon 会被矛盾的路径边界必然标为 incomplete。旧 Signal/Outcome 全部保留审计且不得进入产品分母；本地已将 MFE/MAE 连续路径终点与 forward return 的 eligible evaluation close 统一，必须由新部署 cohort 重评，当前仍不勾选、不调参）
