...
const apiKey =
  keyLine
    ?.slice('COINGECKO_PRO_API_KEY='.length)
    .trim()
    .replace(/^(['"])(.*)\1$/u, '$2') || '';
...
async function request(name, url, options = {}) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`HTTP ${response.status} 非 JSON`);
    }
    if (!response.ok)
    ...
    results.push(result);
    console.log(
      `PASS ${name} (${result.latency_ms}ms${result.count === null ? '' : `, count=${result.count}`})`,
    );
    return json;
  } catch (error) {
    const result = {
      ...
      ok: false,
      latency_ms: Math.round(performance.now() - startedAt),
      error: redactSecrets(error.message, [apiKey]).slice(0, 500),
    };
    results.push(result);
...
async function gatherPools(network) {
  ...
  for (let page = 1; page <= 6 && pools.length < 100; page += 1) {
    const json = await request(
      `pools.list.${network}.page${page}`,
      `${proBase}/onchain/networks/${network}/pools?page=${page}`,
    );
    if (!json) continue;
...
function assertPoolWindows(json, expectedCount) {
  if ((json?.data?.length || 0) !== expectedCount)
    throw new Error(`期望 ${expectedCount} 池，实际 ${json?.data?.length || 0}`);
  for (const item of json.data) {
    ...
    if (!item?.attributes?.net_buy_volume_usd)
      throw new Error(`${item.id} 缺少 net_buy_volume_usd`);
...
function websocketTest(name, channel, code, network, address, extra = {}) {
  return new Promise((resolve) => {
    ...
    const socket = new WebSocket(
      `wss://stream.coingecko.com/v1?x_cg_pro_api_key=${encodeURIComponent(apiKey)}`,
    );
    const finish = (ok, detail) => {
      ...
      const result = {
        ...
        ack,
        ...(ok
          ? { payload_shape: shape(detail) }
          : { error: redactSecrets(detail, [apiKey]).slice(0, 500) }),
      };
      results.push(result);
      console.log(
        `${ok ? 'PASS' : 'FAIL'} ${name} (${result.latency_ms}ms, ack=${ack})${ok ? '' : ` — ${result.error}`}`,
      );
      resolve(result);
    };
        ...
        if (data.type === 'confirm_subscription') {
          socket.send(
            JSON.stringify({
              ...
              identifier,
              data: JSON.stringify({
                'network_id:pool_addresses': [`${network}:${address}`],
                action: 'set_pools',
                ...extra,
              }),
            }),
...
for (const network of ['solana', 'bsc']) {
  ...
  if (pools.length < 50)
    throw new Error(`${network} 只收集到 ${pools.length} 个池，无法验证 50 池上限`);
  ...
  try {
    ...
    results.push({
      name: `pools.multi.50.${network}.assert`,
      ok: false,
      error: redactSecrets(error.message, [apiKey]),
    });
    console.log(
      `FAIL pools.multi.50.${network}.assert — ${redactSecrets(error.message, [apiKey])}`,
    );
  }

  const tokens = [
    ...new Set(pools.flatMap((pool) => [pool.base, pool.quote]).filter(Boolean)),
  ].slice(0, 50);
  if (tokens.length === 50) {
    await request(
    ...
    results.push({
      name: `tokens.multi.50.${network}`,
      ok: false,
      error: `仅收集到 ${tokens.length} 个 token`,
    });
  ...
  await request(
    `pool.single.${network}`,
    `${proBase}/onchain/networks/${network}/pools/${pool.address}?include=base_token,quote_token,dex&include_volume_breakdown=true&include_composition=true`,
  );
  await request(
    `pool.info.${network}`,
    `${proBase}/onchain/networks/${network}/pools/${pool.address}/info`,
  );
  if (pool.base)
    await request(
      `token.info.${network}`,
      `${proBase}/onchain/networks/${network}/tokens/${pool.base}/info`,
    );
  await request(
    `trades.pool.${network}`,
    `${proBase}/onchain/networks/${network}/pools/${pool.address}/trades`,
  );
  await request(
    `ohlcv.second30.base.${network}`,
    `${proBase}/onchain/networks/${network}/pools/${pool.address}/ohlcv/second?aggregate=30&limit=20&currency=usd&token=base`,
  );
  await request(
    `ohlcv.second30.quote.${network}`,
    `${proBase}/onchain/networks/${network}/pools/${pool.address}/ohlcv/second?aggregate=30&limit=20&currency=usd&token=quote`,
  );
  await request(
    `ohlcv.minute1.empty.${network}`,
    `${proBase}/onchain/networks/${network}/pools/${pool.address}/ohlcv/minute?aggregate=1&limit=20&currency=usd&token=base&include_empty_intervals=true`,
  );
}

await Promise.all(
  Object.entries(networkPools).flatMap(([network, pools]) => [
    websocketTest(`ws.G2.${network}`, 'OnchainTrade', 'G2', network, pools[0].address),
    websocketTest(`ws.G3.${network}`, 'OnchainOHLCV', 'G3', network, pools[0].address, {
      interval: '1m',
      token: 'base',
    }),
  ]),
...
const creditDelta =
  Number.isFinite(beforeCalls) && Number.isFinite(afterCalls) ? afterCalls - beforeCalls : null;

await mkdir(resultDir, { recursive: true });
await writeFile(
  new URL('coingecko-contract.json', resultDir),
  `${JSON.stringify(
    {
      ...
      websocket_note:
        'WebSocket messages are billed separately; /key counters may not update immediately.',
      summary: {
