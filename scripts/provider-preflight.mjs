...
async function checkGmgn(apiKey) {
  ...
  for (const chain of ['sol', 'bsc']) {
    for (const interval of ['1m', '5m']) {
      try {
        ...
        fail(`GMGN Trending ${chain} ${interval}`, error, [apiKey]);
      }
    }
  }
  ...
  try {
    ...
    fail('GMGN Hot Searches 1m', error, [apiKey]);
  }
}
...
async function checkCoinGecko(apiKey) {
  ...
  try {
    ...
    pass(
      'CoinGecko /key',
      `${json.plan}; key RPM=${json.api_key_rate_limit_request_per_minute}; key monthly=${json.api_key_monthly_call_credit}; ${elapsedMs}ms`,
    );
  } catch (error) {
    fail('CoinGecko /key', error, [apiKey]);
    return;
  }
  ...
  for (const [network, address] of samples) {
    try {
      ...
      fail(`CoinGecko ${network} 批量池字段`, error, [apiKey]);
    }
  }
}
...
async function checkTelegram(token, values) {
  ...
  try {
    ...
    fail('Telegram getMe', error, [token]);
    return;
  }
  ...
  for (const key of [
    ...
    try {
      ...
      pass(
        `Telegram ${key}`,
        `${json.result.type}: ${json.result.title || json.result.username || json.result.id}`,
      );
    } catch (error) {
      fail(`Telegram ${key}`, error, [token]);
    }
  }
}
