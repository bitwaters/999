...
const quote = 'quote';
const discovery = [
  { chain: 'sol' as const, tokenAddress: token, source: 'trending_1m' as const, observedAt: 1_000, rank: 10 },
  { chain: 'sol' as const, tokenAddress: token, source: 'trending_1m' as const, observedAt: 2_000, rank: 5 },
];
...
test('replay re-runs raw safety, pool, Level 1, ACE, dispatch and entry without live tables', () => {
  ...
  assert.equal((results[0]!.simulatedSignal.snapshot as { age: { status: string } }).age.status, 'pass');
});
...
test('replay warmup seeds cooldown without emitting a pre-window signal', () => {
  ...
  const observations = [
    ...discovery,
    { chain: 'sol' as const, tokenAddress: token, source: 'trending_1m' as const, observedAt: 100_000, rank: 10 },
    { chain: 'sol' as const, tokenAddress: token, source: 'trending_1m' as const, observedAt: 101_000, rank: 5 },
  ];
  const later: ReplayEvidence[] = [
    { kind: 'safety', chain: 'sol', tokenAddress: token, observedAt: 101_000, payload: { renounced_mint: true, renounced_freeze_account: true } },
    ...
    { kind: 'trades', chain: 'sol', tokenAddress: token, poolAddress: pool, observedAt: 104_000, payload: { data: [{ attributes: { block_timestamp: '1970-01-01T00:01:44.000Z' } }] } },
    { kind: 'level1', chain: 'sol', observedAt: 140_000, payload: { data: [level1PoolItem('1.05')] } },
    { kind: 'trades', chain: 'sol', tokenAddress: token, poolAddress: pool, observedAt: 141_000, payload: { data: [{ attributes: { block_timestamp: '1970-01-01T00:02:20.000Z' } }] } },
    { kind: 'g2', chain: 'sol', poolAddress: pool, observedAt: 151_000, payload: { c: 'G2', n: 'solana', pa: pool, ty: 'b', t: 140_000, to: '100', toq: '2000' } },
  ];
  const results = simulateReplay({
    config,
    configVersionId: 7,
...
test('replay parameter changes re-run conviction instead of reusing a live decision', () => {
  ...
  assert.ok(
    (results[0]!.simulatedSignal.reasons as string[]).includes('conviction:rejected'),
  );
});
